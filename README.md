...
- MathJax 3.2.2 (`tex-mml-chtml` + woff-v2 fonts, plus the **mhchem**
  chemistry extension at `input/tex/extensions/mhchem.js`, loaded by every
  theme's MathJax config via `loader.load: ['[tex]/mhchem']`) is bundled once
  at the app level in `3rdparty/mathjax/` and served at `{site}/mathjax/…` by
  a dedicated serve.php branch — deliberately NOT under `themes/` so the site wizard's
  ...
  delimiters; the documentation theme uses the same shared copy (its own
  font-less `js/tex-chtml.js` was dropped 2026-09-15).
